...
- `planning-and-research` — when broader option analysis or research decomposition is needed.
- `csharp-engineering` — recommended implementation owner after the report is complete; not before.
- `architecture-and-ddd` — when remediation requires structural or boundary changes.
